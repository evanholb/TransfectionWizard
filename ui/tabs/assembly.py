...
# Column 1 (A1-D1) = enzymes, Column 2 (A2-D2) = fixed reagents.
# Grouped adjacent for easier physical loading, no functional difference.
ENZYME_WELLS_POOL = REAGENT_WELLS[0:4]
LIGASE_WELL = REAGENT_WELLS[4]
BUFFER_WELL = REAGENT_WELLS[5]
WATER_WELL = REAGENT_WELLS[6]
    ...
    with ui.column().classes('w-full gap-4'):
        ...
        with ui.expansion('', icon='help_outline').classes('w-full'):
            ui.label(
                ...
                '1-6 - they can go in any of those slots.'
            ).classes('text-gray-500 text-sm')
        ...
        def get_unique_enzymes() -> list:
            """Returns unique enzyme names across all rows, in order of first appearance."""
            unique = []
            for row in rows:
        ...
        def get_enzyme_to_well_map():
            """Each unique enzyme gets its own well - no two enzymes share a tube."""
            ...
            with layout_container:
                ...
                with ui.row().classes('gap-4 items-center mb-2 flex-wrap'):
                    for key, label in [('bsai', 'Enzyme'), ('ligase', 'T4 Ligase'), ('buffer', 'Buffer'),
                                        ('water', 'Water'), ('part', 'DNA Part'), ('reaction', 'Assigned Reaction'), ('empty', 'Empty')]:
